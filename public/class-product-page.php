...
class pisol_ppscw_product_page_calculator{
	...
	static function resultHtml(){
		echo '<div id="pisol-ppscw-alert-container" class="pisol-ppscw-alert-container"></div>';
	}
	...
	static function applyShipping(){
		if ( self::doingCalculation() ){
			...
			if(isset($_POST['action_auto_load']) && self::disableAutoLoadEstimate()){
				$return['shipping_methods'] = sprintf('<div class="pisol-ppscw-alert">%s</div>', esc_html(get_option('pi_ppscw_no_address_added_yet', 'Insert your location to get the shipping method')));
				wp_send_json($return);
			}
			/*
			$return = array();
			...
			*/
			...
			if(WC()->cart->get_cart_contents_count() == 0 ){
				...
				$dont_consider_cart_product = get_option('pi_ppscw_ignore_cart_product', 0);

				if(empty($dont_consider_cart_product)){
					$item_key = self::addTestProductForProperShippingCost();
					$packages = WC()->shipping()->get_packages();
				}else{
