...
class pisol_ppscw_popup{
    ...
    function addingBadge(){
        ...
        if(apply_filters('pisol_ppscw_enable_popup',true)){
            ...
            echo '<div id="pisol-ppscw-badge-container" class="pisol-badge-'.esc_attr($this->position).'"><a rel="nofollow" id="pisol-ppscw-badge" href="/" style="background-color:'.esc_attr($bg).'; color:'.esc_attr($color).';">'.wp_kses_post($icon_img).' <span>'.wp_kses_post($this->text).'</span></a></div>';
        }
    }
